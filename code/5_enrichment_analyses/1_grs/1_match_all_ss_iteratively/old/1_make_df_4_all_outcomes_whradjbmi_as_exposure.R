...
data_aligner <- function(query_ss, other_ss){
  ...
  #Just in case let's remove the ones that have NA in the exposure:
  
  query_na <- query_ss[which(is.na(query_ss$beta)),]
  
  merged_df <- merged_df[which(!(merged_df$chr_pos.exposure%in%query_na$chr_pos)),]
  ...
  #If there are NAs from dummy, let's add these variants as if they were missing too!!
  #This solves the issue of missing SNPs:
  
  additional_missing <- query_ss[which(is.na(query_ss$beta))]
  
  missing_snps <- rbind(missing_snps, additional_missing)
  
  missing_snps <- missing_snps[which(duplicated(missing_snps$chr_pos) == FALSE),]
  ...
  if(length(missing_snps$chr_pos) == 0){
    ...
    exposure_match <- exposure_df[which(exposure_df$chr_pos%in%proxies_in_outcome$chr_pos & exposure_df$chr_pos%in%iradjbmi$chr_pos),]
    
    #4. Let's add lead SNP so that we can properly add data:
    
    proxies_match <- proxies_in_outcome[which(proxies_in_outcome$chr_pos%in%exposure_match$chr_pos & proxies_in_outcome$chr_pos%in%iradjbmi$chr_pos),] #we are choosing the proxy that is found in all!! Cuz we need it for mediation.
...
output_path <- "output/5_comparing_fiadjbmi_fi/4_mr/all_variants/1_expo_outcome_df_whradjbmi/"
...
type_of_trait <- c(rep("Cardiometabolic complication", 6))

########################################################
#STEP 1: let's align the cluster to the positive allele#
########################################################

data_df <- whradjbmi_ss[which(whradjbmi_ss$chr_pos%in%all_variants$chr_pos),]
...
#Let's add the missing SNPs as dummies:

missed_variants <- all_variants[which(!(all_variants$chr_pos%in%data_df$chr_pos)),]

dummy_df <- missed_variants %>%
  select(rsid, chr_pos)
colnames(dummy_df) <- c("variant", "chr_pos")

dummy_df$effect_allele <- NA
dummy_df$other_allele <- NA
dummy_df$effect_allele_frequency <- NA
dummy_df$beta <- NA
dummy_df$standard_error <- NA
dummy_df$p_value <- NA
dummy_df$sample_size <- NA
dummy_df$chromosome <- NA
dummy_df$base_pair_location <- NA

data_df <- rbind(data_df, dummy_df)

#######################################
#STEP 1: recursively match ir variants#
#######################################

ir_df <- data_df[which(data_df$variant%in%ir_variants$variant),]

recursively_matching_data(exp_df = ir_df, list_of_phenos_ =list_of_phenos, trait_names_ = trait_names, output_path <- output_path, exposure = "whradjbmi_ir_variants_")
