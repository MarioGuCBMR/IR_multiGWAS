...
chip <- fread("raw_data/chip_atlas/Oth.Adp.05.CEBPB.AllCell.bed", sep = "\t")
colnames(chip) <- c("chr", "start", "stop", "source", "smth_1", "smth_2", "start_2", "stop_2", "rgb_code")

####################################################################
#Let's search if any proxy has an overlap of ChIP in adipocyte data#
####################################################################

proxies$cebpb_chip <- NA
proxies$source <- NA
