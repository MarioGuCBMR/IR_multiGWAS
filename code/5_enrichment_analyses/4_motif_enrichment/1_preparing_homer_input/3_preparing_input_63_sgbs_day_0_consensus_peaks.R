...
peaks <- fread('raw_data/atac_seq_data/GSE178794_SGBS-day0_consensusPeaks-fromUnionRepPeaks_top100k-byMedianPval.bed.gz')
...
length(unique(proxies$query_snp_rsid)) #63 - perfect
...
dir.create("output/5_enrichment_analyses/4_homer/input/63_sgbs_day_0")
dir.create("output/5_enrichment_analyses/4_homer/output/63_sgbs_day_0")

fwrite(unique(IR_peaks), "output/5_enrichment_analyses/4_homer/input/63_sgbs_day_0/IR_peaks.txt", sep = "\t", col.names = F)
...
fwrite(unique(non_IR_peaks), "output/5_enrichment_analyses/4_homer/input/63_sgbs_day_0/non_IR_peaks.txt", sep = "\t", col.names = F)
