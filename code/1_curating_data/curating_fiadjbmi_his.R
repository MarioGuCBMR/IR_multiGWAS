...
fiadjbmi <- fread("raw_data/summary_statistics/GCST90002239_buildGRCh37.tsv.gz") #hist Asian checked: https://www.ebi.ac.uk/gwas/studies/GCST90002239
...
#2. Check the dathisame to see anything suspicious.
...
#1. Always order the dathisame with the variant ID. You might find interesting things.
...
fiadjbmi_corrected <- fiadjbmi_autosomal[which(fiadjbmi_autosomal$effect_allele%in%yes_vect & fiadjbmi_autosomal$other_allele%in%yes_vect),]
...
#We have 2480470 that are NAs.

#################################################################
#We are gonna separate the dathisames to: with NA and without NA#
#################################################################
...
fiadjbmi_end <- fiadjbmi_corrected_eaf[which(!(fiadjbmi_corrected_eaf$chr_pos%in%fiadjbmi_mhc$chr_pos)),]

#########################
#We can save this data!!#
#########################

fwrite(fiadjbmi_end, "output/1_curated_gwas/fiadjbmi_curated_his.txt")
