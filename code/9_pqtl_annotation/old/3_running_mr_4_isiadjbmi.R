...
#This code performs MR  for all cis-pQTL instruments - isiadjbmi systematically.
...
outcome <- fread("output/1_curated_gwas/isiadjbmi_curated.txt")
fiadjbmi <- fread("output/1_curated_gwas/fiadjbmi_curated.txt")

#Let's clean this first:

yes_vect <- c("A", "G", "T", "C")
fiadjbmi <- fiadjbmi[which(fiadjbmi$effect_allele%in%yes_vect & fiadjbmi$other_allele%in%yes_vect),]

fiadjbmi_match <- fiadjbmi[which(fiadjbmi$chr_pos%in%outcome$chr_pos),]
fiadjbmi_match <- fiadjbmi_match[which(duplicated(fiadjbmi_match$chr_pos) == FALSE),]

outcome <- outcome[which(outcome$chr_pos%in%fiadjbmi_match$chr_pos),]
outcome <- outcome[which(duplicated(outcome$chr_pos) == FALSE),]

outcome <- outcome[order(match(outcome$chr_pos, fiadjbmi_match$chr_pos)),]

length(which(outcome$chr_pos == fiadjbmi_match$chr_pos)) #perfect

outcome$variant <- fiadjbmi_match$variant
...
for(protein_ in prot_list){
  ...
  if(length(mr_df$SNP) < 3){
    ...
    dir.create("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi")
    dir.create(paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_, sep = ""))
    
    #save data
    
    saveRDS(mr_steiger_df, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/instruments_after_steiger_df.RDS", sep = ""))
    ...
    saveRDS(mr_res, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/mr_res_before_outlier_extraction.RDS", sep = ""))
  ...
  tiff(filename = paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/plots_before_outlier_extraction.tiff", sep = ""), height=5000, width=5000, res=300)
  mr_plots(mr_steiger_df)
  dev.off()
  
  saveRDS(mr_res, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/mr_res_before_outlier_extraction.RDS", sep = ""))
  saveRDS(rucker, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/sensitivity_test_before_outlier_extraction.RDS", sep = ""))
  saveRDS(strict_het, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/strict_het_test_before_outlier_extraction.RDS", sep = ""))
  ...
  if(length(mr_post$SNP) < 3){ #run WALD or IVW only if less than 3 SNPs
    ...
    saveRDS(mr_post, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/instruments_after_outlier_removal_df.RDS", sep = ""))
    ...
    saveRDS(mr_res, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/mr_res_after_outlier_extraction.RDS", sep = ""))
  ...
  tiff(filename = paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/plots_after_outlier_extraction.tiff", sep = ""), height=5000, width=5000, res=300)
  mr_plots(mr_post)
  dev.off()
  
  saveRDS(mr_res, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/mr_res_after_outlier_extraction.RDS", sep = ""))
  saveRDS(rucker, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/sensitivity_test_after_outlier_extraction.RDS", sep = ""))
  saveRDS(strict_het, paste("output/9_pqtl_results/2_mr/1_mr_res/isiadjbmi/", protein_ , "/strict_het_test_after_outlier_extraction.RDS", sep = ""))
