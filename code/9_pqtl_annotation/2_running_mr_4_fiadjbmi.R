...
#This code performs MR  for all cis-pQTL instruments - FIadjBMI systematically.
...
#Let's load the instruments we found and make some small adjustments so that we can perform MR:

prot_og_df <- fread("output/9_pqtl_results/2_mr/0_data_4_input/all_associations_per_protein_w_more_than_3_credible_sets_18032026.txt") #143
...
#We are also going to get the full list of cis-pQTL matches. We need the lead SNPs the associations are linked to...

pqtl_matched <- fread("output/9_pqtl_results/1_cis_pqtl_matches/cis_pqtl_matches.txt") #23

#Finally, let's get the list of prots that are going to be tested

prot_list <- prot_og_df$protein
prot_list <- unique(prot_list) #20!
prot_list <- prot_list[order(prot_list)]

#Let's load the outcome too:

outcome <- fread("output/1_curated_gwas/fiadjbmi_curated.txt")

#Finally, let's load the proxies: 

proxies <- fread("output/9_pqtl_results/2_mr/0_data_4_input/haploreg_output_18032026.txt", fill = TRUE)
...
length(unique(proxies$query_snp_rsid)) #137/143 - mostly rare variant end up being removed
...
for(protein_ in prot_list){
  ...
  exposure_match = exposure_match[which(exposure_match$ALLELE0%in%yes_vect & exposure_match$ALLELE1%in%yes_vect),]
  ...
  #mr_df <- mr_df[which(as.numeric(mr_df$eaf.outcome) > 0.01 & as.numeric(mr_df$eaf.outcome) < 0.99),]  #we have already filtered this one out...
  ...
  if(length(mr_df$SNP) < 3){
    ...
    dir.create("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026")
    dir.create(paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_, sep = ""))
    
    #save data
    
    saveRDS(mr_steiger_df, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/instruments_after_steiger_df.RDS", sep = ""))
    ...
    saveRDS(mr_res, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/mr_res_before_outlier_extraction.RDS", sep = ""))
  ...
  ggsave(plot=plotio, filename = paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/plots_before_outlier_extraction.svg", sep = ""), height=12, width=12)
  
  saveRDS(mr_res, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/mr_res_before_outlier_extraction.RDS", sep = ""))
  saveRDS(rucker, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/sensitivity_test_before_outlier_extraction.RDS", sep = ""))
  saveRDS(strict_het, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/strict_het_test_before_outlier_extraction.RDS", sep = ""))
  ...
  if(length(mr_post$SNP) < 3){ #run WALD or IVW only if less than 3 SNPs
    ...
    saveRDS(mr_post, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/instruments_after_outlier_removal_df.RDS", sep = ""))
    ...
    saveRDS(mr_res, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/mr_res_after_outlier_extraction.RDS", sep = ""))
  ...
  strict_het_post <- compute_strict_het(mr_post) #they disagree, but still
  ...
  plot_post=mr_plots(mr_post)
  ggsave(plot=plot_post, filename = paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/plots_after_outlier_extraction.svg", sep = ""), height=12, width=12)

  saveRDS(mr_res, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/mr_res_after_outlier_extraction.RDS", sep = ""))
  saveRDS(rucker, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/sensitivity_test_after_outlier_extraction.RDS", sep = ""))
  saveRDS(strict_het_post, paste("output/9_pqtl_results/2_mr/1_mr_res/fiadjbmi_18032026/", protein_ , "/strict_het_test_after_outlier_extraction.RDS", sep = ""))
