...
proxies <- readRDS("6_qtl_annotation/conditional_and_fine_mapped_asat_and_vat_eqtls_sqtls.txt")

#Finally, let's load the QTL data:

qtl_df <- arrow::read_parquet("../../gtex_v10/GTEx_v10_SuSiE_eQTL/Liver.v10.eQTLs.SuSiE_summary.parquet")
...
proxies$gene_id.liver_gtex <- NA
proxies$gene_name.liver_gtex <- NA

proxies$ir_allele.liver_gtex <- NA
proxies$is_allele.liver_gtex <- NA

proxies$afc.liver_gtex <- NA
proxies$standard_error.liver_gtex <- NA
proxies$pip.liver_gtex <- NA
...
for(index in seq(1, length(proxies$rsID))){
  ...
  if(is_empty(qtl_tmp$gene_id_clean)){
    ...
    if(length(variant_otg) == 1| length(lead_otg) == 1){ #one of them have missing info:
      ...
      proxies$gene_id.liver_gtex[index] <- paste(qtl_tmp$gene_id_clean, collapse = ";")
      proxies$gene_name.liver_gtex[index] <- paste(qtl_tmp$gene_name, collapse = ";")
      
      proxies$ir_allele.liver_gtex[index] <- qtl_tmp$alt[1]
      proxies$is_allele.liver_gtex[index] <- qtl_tmp$ref[1]
      
      proxies$afc.liver_gtex[index] <- paste(qtl_tmp$afc, collapse = ";")
      proxies$afc.liver_gtex[index] <- paste("non_aligned", proxies$afc.liver_gtex[index], sep= "_") #adding that the allele might be wrong! Needs to be checked
      
      proxies$standard_error.liver_gtex[index] <- paste(qtl_tmp$afc_se, collapse = ";")
      proxies$pip.liver_gtex[index] <- paste(qtl_tmp$pip, collapse = ";")
      #proxies$cs.liver_gtex[index] <- paste(qtl_tmp$cs_id, collapse = ";")
    ...
    if(af_qtl_res == af_ref_res){
      ...
      proxies$ir_allele.liver_gtex[index] <- qtl_tmp$ref[1]
      proxies$is_allele.liver_gtex[index] <- qtl_tmp$alt[1]
      
      proxies$afc.liver_gtex[index] <- paste(as.numeric(qtl_tmp$afc)*(-1), collapse = ";")
      proxies$standard_error.liver_gtex[index] <- paste(qtl_tmp$afc_se, collapse = ";")
      proxies$pip.liver_gtex[index] <- paste(qtl_tmp$pip, collapse = ";")
      #proxies$cs.liver_gtex[index] <- paste(qtl_tmp$cs_id, collapse = ";")
...
saveRDS(proxies, "6_qtl_annotation/conditional_and_fine_mapped_asat_vat_and_liver_eqtls.txt")
