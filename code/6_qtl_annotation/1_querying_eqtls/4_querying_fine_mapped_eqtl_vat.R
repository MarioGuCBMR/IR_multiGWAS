...
proxies <- readRDS("6_qtl_annotation/conditional_and_fine_mapped_asat_eqtls_sqtls.txt")

#Finally, let's load the QTL data:

qtl_df <- arrow::read_parquet("../../gtex_v10/GTEx_v10_SuSiE_eQTL/Adipose_Visceral_Omentum.v10.eQTLs.SuSiE_summary.parquet")
...
proxies$gene_id.vat_gtex <- NA
proxies$gene_name.vat_gtex <- NA

proxies$ir_allele.vat_gtex <- NA
proxies$is_allele.vat_gtex <- NA

proxies$afc.vat_gtex <- NA
proxies$standard_error.vat_gtex <- NA
proxies$pip.vat_gtex <- NA
...
for(index in seq(1, length(proxies$rsID))){
  ...
  if(is_empty(qtl_tmp$gene_id_clean)){
    ...
    if(length(variant_otg) == 1| length(lead_otg) == 1){ #one of them have missing info:
      ...
      proxies$gene_id.vat_gtex[index] <- paste(qtl_tmp$gene_id_clean, collapse = ";")
      proxies$gene_name.vat_gtex[index] <- paste(qtl_tmp$gene_name, collapse = ";")
      
      proxies$ir_allele.vat_gtex[index] <- qtl_tmp$alt[1]
      proxies$is_allele.vat_gtex[index] <- qtl_tmp$ref[1]
      
      proxies$afc.vat_gtex[index] <- paste(qtl_tmp$afc, collapse = ";")
      proxies$afc.vat_gtex[index] <- paste("non_aligned", proxies$afc.vat_gtex[index], sep= "_") #adding that the allele might be wrong! Needs to be checked
      
      proxies$standard_error.vat_gtex[index] <- paste(qtl_tmp$afc_se, collapse = ";")
      proxies$pip.vat_gtex[index] <- paste(qtl_tmp$pip, collapse = ";")
      #proxies$cs.vat_gtex[index] <- paste(qtl_tmp$cs_id, collapse = ";")
    ...
    if(af_qtl_res == af_ref_res){
      ...
      proxies$ir_allele.vat_gtex[index] <- qtl_tmp$ref[1]
      proxies$is_allele.vat_gtex[index] <- qtl_tmp$alt[1]
      
      proxies$afc.vat_gtex[index] <- paste(as.numeric(qtl_tmp$afc)*(-1), collapse = ";")
      proxies$standard_error.vat_gtex[index] <- paste(qtl_tmp$afc_se, collapse = ";")
      proxies$pip.vat_gtex[index] <- paste(qtl_tmp$pip, collapse = ";")
      #proxies$cs.vat_gtex[index] <- paste(qtl_tmp$cs_id, collapse = ";")
...
saveRDS(proxies, "6_qtl_annotation/conditional_and_fine_mapped_asat_and_vat_eqtls.txt")

check <- readRDS("N:/SUN-CBMR-Kilpelainen-Group/Mario_Tools/IR_GSEM_2025/output/6_qtl_annotation/conditional_and_fine_mapped_asat_and_vat_eqtls.txt")
