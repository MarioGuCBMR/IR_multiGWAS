...
proxies <- readRDS("6_qtl_annotation/conditional_and_fine_mapped_asat_eqtls.txt")
...
qtl_df <- as.data.frame(arrow::read_parquet("../../gtex_v10/GTEx_v10_SuSiE_sQTL/Adipose_Subcutaneous.v10.sQTLs.SuSiE_summary.parquet"))
...
proxies$gene_id.asat_sqtl <- NA
proxies$gene_name.asat_sqtl <- NA
proxies$splicing_region <- NA

proxies$splicing_effect <- NA
proxies$ir_allele.asat_sqtl <- NA
proxies$is_allele.asat_sqtl <- NA

proxies$pip.asat_sqtl <- NA
...
for(index in seq(1, length(proxies$rsID))){
  ...
  if(is_empty(qtl_tmp$gene_id_clean)){
    ...
    if(length(variant_otg) == 1| length(lead_otg) == 1){ #one of them have missing info:
      ...
      proxies$gene_id.asat_sqtl[index] <- paste(qtl_tmp$gene_id_clean, collapse = ";")
      proxies$gene_name.asat_sqtl[index] <- paste(qtl_tmp$gene_name, collapse = ";")
      proxies$splicing_region[index] <- paste(qtl_tmp$splicing, collapse = ";")
      
      proxies$ir_allele.asat_sqtl[index] <- qtl_tmp$alt[1]
      proxies$is_allele.asat_sqtl[index] <- qtl_tmp$ref[1]
      
      proxies$splicing_effect[index] <- paste(qtl_tmp$afc, collapse = ";")
      proxies$splicing_effect[index] <- paste("non_aligned", proxies$splicing_effect[index], sep= "_") #adding that the allele might be wrong! Needs to be checked
      
      proxies$pip.asat_sqtl[index] <- paste(qtl_tmp$pip, collapse = ";")
      #proxies$cs.asat_sqtl[index] <- paste(qtl_tmp$cs_id, collapse = ";")
    ...
    if(af_qtl_res == af_ref_res){
      ...
      proxies$ir_allele.asat_sqtl[index] <- qtl_tmp$ref[1]
      proxies$is_allele.asat_sqtl[index] <- qtl_tmp$alt[1]
      
      proxies$splicing_effect[index] <- paste(as.numeric(qtl_tmp$afc)*(-1), collapse = ";")
      proxies$pip.asat_sqtl[index] <- paste(qtl_tmp$pip, collapse = ";")
      #proxies$cs.asat_sqtl[index] <- paste(qtl_tmp$cs_id, collapse = ";")
...
saveRDS(proxies, "6_qtl_annotation/conditional_and_fine_mapped_asat_eqtls_sqtls.txt")
