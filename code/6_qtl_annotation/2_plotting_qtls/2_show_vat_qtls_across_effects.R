...
retrieving_qtls <- function(ir_set, qtl_set){
  ...
  for(index in seq(1, length(ir_set$variant))){
    ...
    name_vat_gtex <- qtl_info_match$gene_name.vat_gtex[which(str_detect(qtl_info_match$gene_name.vat_gtex, "ENSG") == FALSE)]
    name_vat_sqtl <- qtl_info_match$gene_name.vat_sqt[which(str_detect(qtl_info_match$gene_name.vat_sqtl, "ENSG") == FALSE)]
    
    gene_info <- c(name_vat_gtex, name_vat_sqtl)
    ...
    ir_set$vat_qtl[index] <- gene_info
...
preparing_data <- function(ir_set, pheno_df){
  ...
  ir_classes <- ir_set[, c("variant", "reported_ir", "vat_qtl")]
...
plotting_info_first <- function(combined_df){
  ...
  #Let's remove the vat_qtl with either NAs or with ""
  
  combined_df <- combined_df[which(combined_df$vat_qtl != "" & is.na(combined_df$vat_qtl) == FALSE),]
  
  # Make sure vat_qtl (or gene/SNP label column) is a factor with appropriate order
  combined_df$vat_qtl <- factor(combined_df$vat_qtl, levels = sort(unique(combined_df$vat_qtl)))
  
  # Plot
  plot_transposed <- ggplot(combined_df, aes(x = phenotype_name, y = fct_rev(vat_qtl))) +
...
plotting_info_first_no_x <- function(combined_df){
  ...
  combined_df$phenotype_name <- factor(combined_df$phenotype_name, levels = unique(combined_df$phenotype_name))
  combined_df <- combined_df[which(combined_df$vat_qtl != "" & !is.na(combined_df$vat_qtl)), ]
  combined_df$vat_qtl <- factor(combined_df$vat_qtl, levels = sort(unique(combined_df$vat_qtl)))
  
  ggplot(combined_df, aes(x = phenotype_name, y = fct_rev(vat_qtl))) +
    geom_tile(aes(fill = shape, alpha = p_bin), color = "white", size = 0.5) +
    scale_alpha_manual(values = c("<5E-08" = 1, "<0.05" = 0.6, ">=0.05" = 0.1)) +
    geom_text(data = subset(combined_df, p_bin == "<5E-08"), aes(label = "*"), size = 5) +
    theme_classic() +
...
tiff("output/6_qtl_annotation/vat_qtls_with_effects_on_fat.tiff", height = 9000, width = 9000, res=300)
plot_all
dev.off()
