...
fiadjbmi <- data.table::fread("output/1_curated_gwas/fiadjbmi_curated.txt")

#Let's improve the computing time by removing indels:
...
for(i in seq(1, length(ir_variants$chr_pos))){
  ...
  common_snps <- Reduce(intersect, list(iradjbmi_tmp$chr_pos, fiadjbmi_2_iradjbmi$chr_pos))
  ...
  fiadjbmi_2_iradjbmi <- fiadjbmi_2_iradjbmi[which(fiadjbmi_2_iradjbmi$chr_pos%in%common_snps),]
  fiadjbmi_2_iradjbmi <- organizing_common_data(fiadjbmi_2_iradjbmi, common_snps)
  
  print(length(which(fiadjbmi_2_iradjbmi$chr_pos == iradjbmi_matched_ordered$chr_pos)))
  print(length(which(fiadjbmi_2_iradjbmi$effect_allele == iradjbmi_matched_ordered$effect_allele)))
  
  #####################################################
  #STEP 4: we are ready to set it all up for hyprcoloc#
  #####################################################
  
  betas <- as.matrix(as.data.frame(cbind(iradjbmi_matched_ordered$beta, fiadjbmi_2_iradjbmi$beta)))
  ses <- as.matrix(as.data.frame(cbind(iradjbmi_matched_ordered$standard_error, fiadjbmi_2_iradjbmi$standard_error)))
  traits <- c("IRadjBMI", "FIadjBMI")
...
fwrite(coloc_df, "output/4_ir_loci_discovery/3_novel_variants/4_colocalization/282_coloc_fiadjbmi.txt")

check <- fread("output/4_ir_loci_discovery/3_novel_variants/4_colocalization/282_coloc_fiadjbmi.txt")


#############################################################################################################
#STEP 3: lets's add info from the novel hits and the 352 so that we can add this data on supplementary table#
#############################################################################################################

all_variants <- fread("output/4_ir_loci_discovery/2_ir_variants/282_ir_fiadjbmi_hdl_tg_dwls_snps_w_cpassoc.txt")

ir_variants_ <- ir_variants[, c("variant", "reported_ir", "ir_source", "t2d", "t2d_cluster", "loci", "nearest_gene")]
ir_variants_ <- merge(all_variants, ir_variants_, by.x = "variant", by.y = "variant", all.x = T) ; nrow(all_variants) ; head(all_variants, 3)

#Now let's loop over the data and add the coloc info:

ir_variants_$post_prob_coloc.fiadjbmi <- NA
ir_variants_$driver_coloc.fiadjbmi <- NA
...
for(i in seq(1, length(ir_variants_$variant))){
  ...
  if(is_empty(coloc_tmp$iteration)){
    ...
    ir_variants_$post_prob_coloc.fiadjbmi[i]  <- coloc_tmp$posterior_prob
    ...
    ir_variants_$driver_coloc.fiadjbmi[i]  <- candidate_snp
...
fwrite(ir_variants_, "output/4_ir_loci_discovery/3_novel_variants/4_colocalization/352_variants_with_all_info_plus_fiadjbmi_coloc.txt")
