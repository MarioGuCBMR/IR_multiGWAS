...
fwrite(as.data.frame(ir_variants$variant), "output/4_ir_loci_discovery/4_proxies/282_ir_4_proxies.txt", quote=FALSE, row.names = FALSE, col.names = FALSE, sep = " ")

#########################################################################################################
#Let's load all the proxies dataframes, see if they have differences and add those that might be missing#
#########################################################################################################

proxies_1 <- fread("output/4_ir_loci_discovery/4_proxies/282_proxies_core_15.txt", fill = TRUE)
proxies_2 <- fread("output/4_ir_loci_discovery/4_proxies/282_proxies_core_25.txt", fill = TRUE)
proxies_3 <- fread("output/4_ir_loci_discovery/4_proxies/282_proxies_h3k4me.txt", fill = TRUE)
proxies_4 <- fread("output/4_ir_loci_discovery/4_proxies/282_proxies_h3k27ac_9ac.txt", fill = TRUE)

#Let's ensure the data is the same:

length(which(proxies_1$rsID%in%proxies_2$rsID))
length(which(proxies_1$rsID%in%proxies_3$rsID))
length(which(proxies_1$rsID%in%proxies_4$rsID))

#yes, we will proceed with proxy_1

#Are all the query SNPs there?

length(which(ir_variants$variant%in%proxies_1$query_snp_rsid)) #yes
length(which(ir_variants$variant%in%proxies_1$rsID)) #yes
...
proxies_missing <- proxies_clean[which(is.na(proxies_clean$pos_hg19)),] #371! Quite mane
proxies_good <- proxies_clean[which(is.na(proxies_clean$pos_hg19)==FALSE),] #371! Quite mane
...
fwrite(proxies_end_, "output/4_ir_loci_discovery/4_proxies/proxies_build_37_and_38_4_282_variants.txt")

check <- data.table::fread("output/4_ir_loci_discovery/4_proxies/proxies_build_37_and_38_4_282_variants.txt")
...
check_weird <- check[which(is.na(check$pos_hg19)),]
...
data.table::fwrite(final_check, "output/4_ir_loci_discovery/4_proxies/proxies_build_37_and_38_4_282_variants.txt")
